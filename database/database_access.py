...
class database_conn:
    ...
    def _create_user_login_db(self):
        ...
        self.cur.execute("CREATE TABLE users_login (user_id SERIAL PRIMARY KEY, username VARCHAR(255) NOT NULL, password VARCHAR(255) NOT NULL);")
...
def add_user_login(self, user_id: int, username: str, password: str):
    """
    ...
    if count == 0:
        ...
        self.cur.execute("INSERT INTO users_login (user_id, username, password) VALUES (%s, %s, %s);", (user_id, username, password))
    else:
        # The user exists, update the existing record
        self.cur.execute("UPDATE users_login SET username = %s, password = %s WHERE user_id = %s;", (username, password, user_id))
    self.conn.commit()
...
def get_user_login(self, user_id: int):
    ...
    """
    self.cur.execute("SELECT username, password FROM users_login WHERE user_id = %s;", (user_id,))
...
obj = database_conn()
obj.add_user_interest(122, "politics")
print(obj.get_user_interests(122))
obj.conn.close()
